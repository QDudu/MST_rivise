...
def prepare_data(path, file_num):
    HR_HSI = np.zeros((((512,512,23,file_num))))
...
def prepare_data_cave(path, file_num):
    ...
    for idx in range(file_num):
        ...
        data = sio.loadmat(path1)
        HR_HSI[:,:,:,idx] = data['data_slice'] [:,:,:23]/ 65535.0
        HR_HSI[HR_HSI < 0] = 0
        HR_HSI[HR_HSI > 1] = 1
    return HR_HSI
'''
...
def prepare_data_cave(path, file_num):
    HR_HSI = np.zeros((((512,512,10,file_num))))
    file_list = os.listdir(path)
    # for idx in range(1):
    for idx in range(file_num):
        print(f'loading CAVE {idx}')
        ...
        data = sio.loadmat(path1)
        # 都只要前10个波段
        HR_HSI[:,:,:,idx] = data['data_slice'] [:,:,:10]/ 65535.0
        HR_HSI[HR_HSI < 0] = 0
        HR_HSI[HR_HSI > 1] = 1
    return HR_HSI

def prepare_data_KAIST(path, file_num):
    HR_HSI = np.zeros((((2704,3376,10,file_num))))
    file_list = os.listdir(path)
    # for idx in range(1):
    for idx in range(file_num):
