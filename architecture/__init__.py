...
# 28->10 add nC = 10
def model_generator(method, pretrained_model_path=None, nC=10):
    if method == 'mst':
        model = MST(dim=10, stage=2, num_blocks=[2, 2, 2],nC=nC).cuda()
    elif method == 'mst_m':
        model = MST(dim=10, stage=2, num_blocks=[2, 4, 4],nC=nC).cuda()
    elif method == 'mst_l':
        model = MST(dim=10, stage=2, num_blocks=[4, 7, 5],nC=nC).cuda()
    elif method == 'gap_net':
        model = GAP_net().cuda()
